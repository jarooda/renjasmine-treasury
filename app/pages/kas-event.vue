...
useHead({
  title: "Kas Event - Treasury Management",
});
...
const kasEventData = reactive<KasEvent[]>([]);

const {
  data: kasEventResponse,
  pending: isLoading,
  error,
} = await useFetch<SheetResponse<KasEvent[]>>("/api/gsheet", {
  query: { sheet: "Kas Event" },
  transform: (data) => {
...
if (error.value) {
  showError();
} else if (kasEventResponse.value) {
  kasEventData.splice(0, kasEventData.length, ...kasEventResponse.value.data);
}
</script>
    ...
    <div class="max-w-7xl mx-auto py-4 px-3 sm:py-6 sm:px-4 lg:px-6">
      ...
      <UCard>
        <template #header>
          <div class="flex items-center justify-between">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">
              Data Transaksi Kas Event
            </h3>
            <div
              class="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400"
            >
              <UIcon name="i-mdi-table" class="w-4 h-4" />
              <span>{{ kasEventData.length }} transaksi</span>
            </div>
          </div>
        </template>
        ...
        <div class="hidden lg:block overflow-x-auto">
          ...
          >
            ...
            >
              ...
              <tr
                v-for="(transaction, index) in kasEventData"
                :key="index"
        ...
        <div class="lg:hidden space-y-4">
          ...
          <div
            v-if="!isLoading && kasEventData.length === 0"
