...
// Meta tags
useHead({
  title: "Kas RT - Treasury Management",
});
...
const kasRTData = reactive<KasRT[]>([]);

const {
  data: kasRTResponse,
  pending: isLoading,
  error,
} = await useFetch<SheetResponse<KasRT[]>>("/api/gsheet", {
  query: { sheet: "Kas RT" },
  transform: (data) => {
...
if (error.value) {
  showError();
} else if (kasRTResponse.value) {
  kasRTData.splice(0, kasRTData.length, ...kasRTResponse.value.data);
}
</script>
    ...
    <div class="max-w-7xl mx-auto py-4 px-3 sm:py-6 sm:px-4 lg:px-6">
      ...
      <div class="mb-6">
        ...
        <p class="text-gray-600 dark:text-gray-400">
          Detail transaksi kas rukun tetangga
        </p>
      </div>
      ...
      <UCard>
        <template #header>
          <div class="flex items-center justify-between">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">
              Data Transaksi Kas RT
            </h3>
            <div
              class="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400"
            >
              <UIcon name="i-mdi-table" class="w-4 h-4" />
              <span>{{ kasRTData.length }} transaksi</span>
            </div>
          </div>
        </template>
        ...
        <div class="hidden lg:block overflow-x-auto">
          ...
          >
            ...
            >
              ...
              <tr
                v-for="(transaction, index) in kasRTData"
                :key="index"
        ...
        <div class="lg:hidden space-y-4">
          ...
          <div
            v-if="!isLoading && kasRTData.length === 0"
